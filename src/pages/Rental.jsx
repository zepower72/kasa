...
/*Ainsi, la condition rental.id === id.slice(1)rendue si la propriété idde chaque élément rentalest égale à la chaîne de caractères obtenue en enlevant 
le premier caractère de id. Si une correspondance est trouvée, findrenverra cet élément rental.
En conclusion, cette ligne de code recherche dans le tableau dataRentun élément dont la propriété idcorrespond à une partie spécifique de la variable id
(à partir du deuxième caractère). Le résultat est stocké dans la constante rent.*/
    ...
    Cette condition effectuée si la variable rentest fausse ou nulle.*/
    /*Si rentest renvoyé à falseou null, cela signifie qu'aucun emplacement n'a été trouvé (peut-être à partir de la ligne de code que nous avons évoquée précédemment). 
    Dans ce cas, la fonction navigateest appelée avec l'argument "/*". 
